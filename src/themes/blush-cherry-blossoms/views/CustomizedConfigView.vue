...
<template>
  <DashboardLayout>
    <SakuraCard title="自定义配置" subtitle="管理 Clash 等客户端配置文件" accent>
      <template #actions>
        <SakuraBtn size="sm" @click="openNew">新建</SakuraBtn>
      </template>
      ...
      <div class="config-list">
        <article v-for="item in filtered" :key="item.type + item.name" class="config-item">
          ...
          <div class="config-item__actions">
            <SakuraBtn variant="text" size="sm" @click="openView(item)">查看</SakuraBtn>
            <SakuraBtn variant="text" size="sm" @click="openEdit(item)">编辑</SakuraBtn>
            <SakuraBtn variant="text" size="sm" @click="remove(item)">删除</SakuraBtn>
          </div>
        </article>
    ...
    <SakuraModal v-model="editorVisible" :title="editorTitle" wide>
      <div class="form">
      ...
      <template #footer>
        <SakuraBtn variant="ghost" @click="editorVisible = false">取消</SakuraBtn>
        <SakuraBtn @click="save">保存</SakuraBtn>
      </template>
    </SakuraModal>

    <SakuraModal v-model="viewerVisible" :title="editing.name" wide>
      <pre class="viewer">{{ editing.value }}</pre>
      <template #footer>
        <SakuraBtn @click="touchCopy(editing.value)">复制</SakuraBtn>
      </template>
    </SakuraModal>
  </DashboardLayout>
</template>
